...
public class BarGraph extends View implements HoloGraphAnimate {
    ...
    public void onDraw(Canvas canvas) {
        ...
        if (mShowAxis) {
            ...
            mPaint.setStrokeWidth(1 * resources.getDisplayMetrics().density);
        ...
        float barWidth = (getWidth() - 2*PADDING_LEFT_RIGHT*resources.getDisplayMetrics().density - specialWidthTotal - (padding * 2 *normalCount)) / (normalCount);//calculate regular widths
        ...
        count = 0;
        int oldright = (int) (padding *-1) + (int)(PADDING_LEFT_RIGHT*resources.getDisplayMetrics().density);
        ...
        for (final Bar bar : mBars) {
        	...
        	float usableHeight;
            if (mShowBarText && !bar.getValueString().equals("")) {
                ...
                usableHeight = getHeight() - bottomPadding - 9 * resources.getDisplayMetrics().density;
            }
            ...
            if (mShowAxisLabel) {
                ...
                if (isAnimating()) mPaint.setAlpha(alpha);
                float textWidth = mPaint.measureText(bar.getName());
                int x = (int) (((mBoundsRect.left + mBoundsRect.right) / 2) - (textWidth / 2.4f));
                int y = (int) (getHeight() - 6 * resources.getDisplayMetrics().scaledDensity);
                if(labelTypeface!=null) mPaint.setTypeface(labelTypeface);
            ...
            if (mShowBarText) {
                ...
                if(!bar.getValueString().equals(""))
                	mPaint.getTextBounds(bar.getValueString(), 0, 1, mTextRect);
                else
                	mTextRect = new Rect(0, 0, 0, 0);
                ...
                if(valueTypeface!=null) mPaint.setTypeface(valueTypeface);

                if (isAnimating()) mPaint.setAlpha(popupAlpha);
                canvas.drawText(bar.getValueString(),
                        (int) (((mBoundsRect.left + mBoundsRect.right) / 2)
                                - (mPaint.measureText(bar.getValueString())) / 2.3f),
